...
class Application(tk.Frame):
    ...
    def __init__(self, master):
        ...
        message1 = tk.Message(frame1, width=MSG_WIDTH,
                              text=NOTES_1, font=FONT_NOTE)
        message1.grid()
        message2 = tk.Message(frame1, width=MSG_WIDTH,
                              text=NOTES_2, font=FONT_NOTE)
        ...
        self.label22 = tk.Label(frame2, relief=tk.SUNKEN,
                                bg='white', width=40)
        self.label22.grid(row=0, column=1, padx=5)

        btn_file_select = tk.Button(frame2, text='...',
                                    width=4, command=self.file_select)
    ...
    def generate_b4csv(self):
        ...
        if os.path.isfile(self.adi_file):
            ...
            if os.path.isfile(self.csv_file):
                ...
                cp, _ = os.path.split(__file__)
                if not os.path.isdir(f'{cp}/backup'):
                    os.mkdir(f'{cp}/backup')
                _ts = dt_now.strftime("%Y%m%d%H%M%S")
                dst_filename = f'{cp}/backup/{_ts}_B4.csv'
